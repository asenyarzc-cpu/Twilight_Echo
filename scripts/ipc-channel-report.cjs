...
const IPC_CONSTANTS = (() => {
  // Normalize line endings: the entry regexes anchor with `$`, so a CRLF
  // checkout leaves a trailing `\r` on every line and silently drops the whole
  // constant table ("Unknown IPC channel constant reference" for every use).
  const source = fs
    .readFileSync(path.join(ROOT, 'src', 'shared', 'ipcChannels.ts'), 'utf8')
    .replace(/\r\n/g, '\n')
  const map = new Map()
  let domain = null
